...
!
      ...
      public :: init_forcing_atmo, init_forcing_ocn, alloc_forcing, &
                ...
                init_snowtable, get_forcing_bry
...
!=======================================================================
      ...
      use ice_diagnostics, only: debug_model_step
! subroutine to read boundary data, not all tracers are supported
      use ice_constants, only: field_loc_center, field_type_scalar
      use ice_calendar, only: days_per_year
      use ice_domain, only: bdy_origin,nblocks
      use icepack_tracers, only: tr_pond_lvl, tr_FY, tr_lvl,tr_iage
     ...
     !-------------------------------------------------------------------
     call icepack_query_parameters(secday_out=secday)
     !initializing the bdy arrays:
      if (.not. allocated(aicen_bry)) then
          allocate(aicen_bry(nx_block,ny_block,ncat,max_blocks))
          allocate(vicen_bry(nx_block,ny_block,ncat,max_blocks))
          allocate(vsnon_bry(nx_block,ny_block,ncat,max_blocks))
          allocate(Tsfc_bry(nx_block,ny_block,ncat,max_blocks))
          allocate(Tinz_bry(nx_block,ny_block,nilyr,ncat,max_blocks))
          allocate(Sinz_bry(nx_block,ny_block,nilyr,ncat,max_blocks))
          allocate(qsno_bry(nx_block,ny_block,nslyr,ncat,max_blocks))
          allocate(uvel_bry(nx_block,ny_block,max_blocks))
          allocate(vvel_bry(nx_block,ny_block,max_blocks))
          allocate(scale_factor_bry(nx_block,ny_block,max_blocks))
          allocate(swvdr_bry(nx_block,ny_block,max_blocks))
          allocate(swvdf_bry(nx_block,ny_block,max_blocks))
          allocate(swidr_bry(nx_block,ny_block,max_blocks))
          allocate(swidf_bry(nx_block,ny_block,max_blocks))
          allocate(strocnxT_bry(nx_block,ny_block,max_blocks))
          allocate(strocnyT_bry(nx_block,ny_block,max_blocks))
          allocate(stressp_1_bry(nx_block,ny_block,max_blocks))
          allocate(stressp_2_bry(nx_block,ny_block,max_blocks))
          allocate(stressp_3_bry(nx_block,ny_block,max_blocks))
          allocate(stressp_4_bry(nx_block,ny_block,max_blocks))
          allocate(stressm_1_bry(nx_block,ny_block,max_blocks))
          allocate(stressm_2_bry(nx_block,ny_block,max_blocks))
          allocate(stressm_3_bry(nx_block,ny_block,max_blocks))
          allocate(stressm_4_bry(nx_block,ny_block,max_blocks))
          allocate(stress12_1_bry(nx_block,ny_block,max_blocks))
          allocate(stress12_2_bry(nx_block,ny_block,max_blocks))
          allocate(stress12_3_bry(nx_block,ny_block,max_blocks))
          allocate(stress12_4_bry(nx_block,ny_block,max_blocks))
          allocate(iceumask_bry(nx_block,ny_block,max_blocks))
          allocate(frz_onset_bry(nx_block,ny_block,max_blocks))
          allocate(vlvl_bry(nx_block,ny_block,ncat,max_blocks))
          allocate(alvl_bry(nx_block,ny_block,ncat,max_blocks))
          allocate(fsnow_bry(nx_block,ny_block,max_blocks))
          allocate(frzmelt_bry(nx_block,ny_block,max_blocks))
          allocate(sst_bry(nx_block,ny_block,max_blocks))
          allocate(ipnd_bry(nx_block,ny_block,ncat,max_blocks))
          allocate(hpnd_bry(nx_block,ny_block,ncat,max_blocks))
          allocate(apnd_bry(nx_block,ny_block,ncat,max_blocks))
          allocate(dhs_bry(nx_block,ny_block,ncat,max_blocks))
          allocate(ffrac_bry(nx_block,ny_block,ncat,max_blocks))
          allocate(FY_bry(nx_block,ny_block,ncat,max_blocks))
          allocate(iage_bry(nx_block,ny_block,ncat,max_blocks))
      endif
      ...
      if (bdy_origin=='restart_f') then
        !write (nu_diag,*) 'msec: ',msec
        write(nchary,'(i4.4)') myear
        write(ncharm,'(i2.2)') mmonth
        write(nchard,'(i2.2)') mday
        write(nchars,'(i5.5)') msec
        data_file='cice_bdy_restart'//trim(nchary)//trim(ncharm)//trim(nchard)//trim(nchars)//'.nc'
        nrec=1
        dbug = .false.
        ...
        fieldname1='aicen'
        call ice_read_nc &
              (fid, nrec, fieldname1,aicen_bry(:,:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='Tsfcn'
        call ice_read_nc &
              (fid, nrec, fieldname1,Tsfc_bry(:,:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)
        if (oceanmixed_ice) then ! field needed by ocean mixed layer 
                fieldname1='frzmlt'
                call ice_read_nc &
                       (fid, nrec, fieldname1,frzmelt_bry(:,:,:), dbug, &
                        field_loc_center, field_type_scalar,restart_ext)
                fieldname1='sst'
                call ice_read_nc &
                       (fid, nrec, fieldname1,sst_bry(:,:,:), dbug, &
                        field_loc_center, field_type_scalar,restart_ext)
        endif

        fieldname1='iceumask'
        call ice_read_nc &
              (fid, nrec, fieldname1,iceumask_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        do k=1,nilyr
          write(nchar,'(i3.3)') k

          fieldname1='sice'//trim(nchar)
          call ice_read_nc &
                (fid, nrec,fieldname1,Sinz_bry(:,:,k,:,:), dbug, &
                 field_loc_center, field_type_scalar,restart_ext)

          fieldname1='qice'//trim(nchar)
          call ice_read_nc &
                (fid, nrec, fieldname1,Tinz_bry(:,:,k,:,:), dbug, &
                 field_loc_center, field_type_scalar,restart_ext)
        enddo

        do k=1,nslyr
          write(nchar,'(i3.3)') k
          fieldname1='qsno'//trim(nchar)
          call ice_read_nc &
                (fid, nrec, fieldname1,qsno_bry(:,:,k,:,:), dbug, &
                 field_loc_center, field_type_scalar,restart_ext)
        enddo

        fieldname1='scale_factor'
        call ice_read_nc &
              (fid, nrec, fieldname1,scale_factor_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='stress12_1'
        call ice_read_nc &
              (fid, nrec, fieldname1,stress12_1_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='stress12_2'
        call ice_read_nc &
              (fid, nrec, fieldname1,stress12_2_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='stress12_3'
        call ice_read_nc &
              (fid, nrec, fieldname1,stress12_3_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='stress12_4'
        call ice_read_nc &
              (fid, nrec, fieldname1,stress12_4_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='stressm_1'
        call ice_read_nc &
              (fid, nrec, fieldname1,stressm_1_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='stressm_2'
        call ice_read_nc &
              (fid, nrec, fieldname1,stressm_2_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='stressm_3'
        call ice_read_nc &
              (fid, nrec, fieldname1,stressm_3_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='stressm_4'
        call ice_read_nc &
              (fid, nrec, fieldname1,stressm_4_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='stressp_1'
        call ice_read_nc &
              (fid, nrec, fieldname1,stressp_1_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='stressp_2'
        call ice_read_nc &
              (fid, nrec, fieldname1,stressp_2_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='stressp_3'
        call ice_read_nc &
              (fid, nrec, fieldname1,stressp_3_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='stressp_4'
        call ice_read_nc &
              (fid, nrec, fieldname1,stressp_4_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='strocnxT'
        call ice_read_nc &
              (fid, nrec, fieldname1,strocnxT_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='strocnyT'
        call ice_read_nc &
              (fid, nrec, fieldname1,strocnyT_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='swidf'
        call ice_read_nc &
              (fid, nrec, fieldname1,swidf_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='swidr'
        call ice_read_nc &
              (fid, nrec, fieldname1,swidr_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='swvdf'
        call ice_read_nc &
              (fid, nrec, fieldname1,swvdf_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='swvdr'
        call ice_read_nc &
              (fid, nrec, fieldname1,swvdr_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='uvel'
        call ice_read_nc &
              (fid, nrec, fieldname1,uvel_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='vicen'
        call ice_read_nc &
              (fid, nrec, fieldname1,vicen_bry(:,:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='vsnon'
        call ice_read_nc &
              (fid, nrec, fieldname1,vsnon_bry(:,:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)

        fieldname1='vvel'
        call ice_read_nc &
              (fid, nrec, fieldname1,vvel_bry(:,:,:), dbug, &
               field_loc_center, field_type_scalar,restart_ext)
        if (tr_pond_lvl) then
                fieldname1='ipnd'
                call ice_read_nc &
                        (fid, nrec, fieldname1,ipnd_bry(:,:,:,:), dbug, &
                         field_loc_center, field_type_scalar,restart_ext)
                fieldname1='hpnd'
                call ice_read_nc &
                        (fid, nrec, fieldname1,hpnd_bry(:,:,:,:), dbug, &
                         field_loc_center, field_type_scalar,restart_ext)
                fieldname1='apnd'
                call ice_read_nc &
                        (fid, nrec, fieldname1,apnd_bry(:,:,:,:), dbug, &
                         field_loc_center, field_type_scalar,restart_ext)
                fieldname1='fsnow'
                call ice_read_nc &
                        (fid, nrec, fieldname1,fsnow_bry(:,:,:), dbug, &
                         field_loc_center, field_type_scalar,restart_ext)
                fieldname1='dhs'
                call ice_read_nc &
                        (fid, nrec, fieldname1,dhs_bry(:,:,:,:), dbug, &
                         field_loc_center, field_type_scalar,restart_ext)
                fieldname1='ffrac'
                call ice_read_nc &
                        (fid, nrec, fieldname1,ffrac_bry(:,:,:,:), dbug, &
                         field_loc_center, field_type_scalar,restart_ext)
        endif
        if (tr_FY) then
                fieldname1='frz_onset'
                call ice_read_nc &
                        (fid, nrec, fieldname1,frz_onset_bry(:,:,:), dbug, &
                         field_loc_center, field_type_scalar,restart_ext)
                fieldname1='FY'
                call ice_read_nc &
                        (fid, nrec, fieldname1,FY_bry(:,:,:,:), dbug, &
                         field_loc_center, field_type_scalar,restart_ext)
        endif
        if (tr_lvl) then
                fieldname1='vlvl'
                call ice_read_nc &
                        (fid, nrec, fieldname1,vlvl_bry(:,:,:,:), dbug, &
                         field_loc_center, field_type_scalar,restart_ext)
                fieldname1='alvl'
                call ice_read_nc &
                        (fid, nrec, fieldname1,alvl_bry(:,:,:,:), dbug, &
                         field_loc_center, field_type_scalar,restart_ext)
        endif
        if (tr_iage) then
                fieldname1='iage'
                call ice_read_nc &
                        (fid, nrec, fieldname1,iage_bry(:,:,:,:), dbug, &
                         field_loc_center, field_type_scalar,restart_ext)
        endif
       else
              write(nu_diag,*) 'bdy_origin setting not supported'
       endif
